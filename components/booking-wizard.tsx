...
export function BookingWizard() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/availability?anchor=${encodeURIComponent(form.date)}`, { signal: controller.signal })
      .then(async (response) => {
  ...
  if (sent) {
    return (
      <div className="booking-success">
        ...
        <h2>預約需求已送出</h2>
        <p>這筆預約現在會先保留對應時段，其他顧客會看到灰色「保留中」。77 確認後，該時段會從可預約清單移除。</p>
        ...
        {step === 2 && (
          <section className="booking-step booking-time-step">
            ...
            <div className="availability-legend" aria-label="時段狀態說明">
              ...
              <span><i className="legend-dot held" />其他顧客預約中，暫時保留</span>
            ...
            <div className="notice-box booking-rule-note">
              ...
              <p>每 30 分鐘顯示一格。所有服務暫定 90 分鐘，另外保留 30 分鐘整理緩衝，因此選 13:00 時會先保留 13:00、13:30、14:00、14:30 四格。顧客剛送出時四格會反灰；77 後台確認後四格會直接從可預約清單移除。</p>
            </div>
          </section>
        )}
        ...
        {step === 4 && (
          <section className="booking-step">
            ...
            <dl className="summary-list">
              ...
              <div><dt>預留</dt><dd>90 分鐘服務＋30 分鐘整理緩衝</dd></div>
              <div><dt>姓名</dt><dd>{form.name || "尚未填寫"}</dd></div>
              <div><dt>首次來店</dt><dd>{form.firstVisit === "yes" ? "是" : "否"}</dd></div>
            </dl>
            <label className="check-row"><input type="checkbox" checked={form.privacy} onChange={(event) => update("privacy", event.target.checked)} /><span>我同意 77美學工作室為處理本次預約而使用我填寫的聯絡資料。</span></label>
            <div className="notice-box"><strong>預約不是送出即成立</strong><p>送出後會先暫時保留時段並顯示灰色。77 確認後，該時段才會從其他顧客的可預約清單移除；首次預約確認後再通知訂金支付。</p></div>
          </section>
        )}

        {message && <p className="form-message">{message}</p>}
        <div className="booking-actions">
          {step > 1 && <button type="button" className="button text" onClick={() => { setMessage(""); setStep((current) => current - 1); }}>上一步</button>}
