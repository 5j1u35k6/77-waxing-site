...
const CATALOG = [
  {
    ...
    note: "目前未提供各熱蠟細項的正式施作時間，先沿用既有 90 分鐘預估。",
  },
  {
    key: "men",
    name: "男士熱蠟",
...
function bookingMarkup() {
  return `<div class="steps"><span class="on">1 服務</span><span>2 日期時段</span><span>3 資料</span><span>4 確認</span></div>
    <section class="step on" data-step="1">
      <h2>選擇服務項目</h2>
      <p class="muted">先選左側服務分類，再選右側細項；細項會顯示施作時間。</p>
...
async function mount(root) {
  if (!root || root.dataset.bookingV2Mounted === "1") return;
  if (!firebaseConfigured) return;
  root.dataset.bookingV2Mounted = "1";
