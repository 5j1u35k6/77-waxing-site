...
async function renderSettings(){
  ...
  const start=live.bookingStartTime||"10:00",end=live.bookingEndTime||"20:00",max=Number(live.maxAdvanceDays||60),enabled=live.bookingEnabled!==false,notice=live.bookingNotice||"";
  const timeOptions=Array.from({length:21},(_,i)=>{const mins=600+i*30;return `${pad(Math.floor(mins/60))}:${pad(mins%60)}`;});
