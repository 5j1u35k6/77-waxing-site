...
async function enhance(){
  ...
  box.innerHTML=`<div class="admin-view-head"><div><span class="tag">EMAIL</span><h4>Email 與訂金通知</h4></div></div><div class="settings-grid"><label>店家收件／寄件 Email<input type="email" data-store-email value="${safe(s.storeEmail)}" placeholder="例如 hello@example.com"></label><label class="full">Google Apps Script Web App 網址<input type="url" data-apps-script-email-url value="${safe(s.appsScriptEmailUrl)}" placeholder="https://script.google.com/macros/s/.../exec"></label><label class="full">訂金 QR Code 圖片網址<input type="url" data-deposit-qr value="${safe(s.depositQrUrl)}" placeholder="https://..."></label></div><div class="actions"><button class="btn dark" type="button" data-email-settings-save>儲存 Email 設定</button><span class="muted" data-email-settings-message></span></div><p class="muted">Email 由 Google Apps Script 以你的 Google 帳號寄出，不需要 Firebase Blaze，也不需要把 Gmail 密碼存進網站。</p>`;
  w.appendChild(box);
  box.querySelector('[data-email-settings-save]').onclick=async()=>{
    ...
    const storeEmail=box.querySelector('[data-store-email]').value.trim();
    ...
    try{await setDoc(doc(firestore,'settings','general'),{storeEmail,appsScriptEmailUrl:appsScriptEmailUrl||null,depositQrUrl:depositQrUrl||null,updatedAt:serverTimestamp()},{merge:true});msg.textContent='已儲存。';}
