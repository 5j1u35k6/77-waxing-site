...
async function enqueue(id, to, subject, html) {
  if (!to) return;
  await db.doc(`mail/${id}`).set({ to: [to], message: { subject, html }, createdAt: FieldValue.serverTimestamp() }, { merge: false });
}
...
exports.bookingCreatedEmail = onDocumentCreated('bookings/{bookingId}', async (event) => {
  ...
  if (customerEmail) {
    await enqueue(`${id}-received-customer`, customerEmail, '77美學工作室｜已收到你的預約需求', shell('已收到你的預約需求', `<p>你的預約需求已送出，目前正在等待店家確認。</p>${bookingTable(b)}<p>確認完成後，我們會再寄一封 Email 通知你。</p>`));
  }
  if (storeEmail) {
    await enqueue(`${id}-received-store`, storeEmail, `新預約待確認｜${b.customerName || '未命名'}｜${b.preferredDate || ''} ${b.preferredTime || ''}`, shell('有新的預約需求', `${bookingTable(b)}${line('手機', b.customerPhone || '—')}${line('Email', customerEmail || '—')}<p>請至管理後台確認預約。</p>`));
  }
});
...
exports.bookingUpdatedEmail = onDocumentUpdated('bookings/{bookingId}', async (event) => {
  ...
  if (after.status === 'pending_payment' || after.status === 'confirmed') {
    ...
    await enqueue(`${id}-${after.status}-customer`, customerEmail, needsDeposit ? '77美學工作室｜預約已確認，請完成訂金' : '77美學工作室｜預約已確認', shell(needsDeposit ? '預約可安排｜等待訂金' : '預約已確認', body));
  }

  if (after.status === 'cancelled' && after.rejectionReasonCode) {
    await enqueue(`${id}-rejected-customer`, customerEmail, '77美學工作室｜預約安排通知', shell('這次預約無法安排', `<p>${esc(rejectionText(after))}</p>${bookingTable(after)}<p>謝謝你的理解。</p>`));
  }
});
